...
class ISMReelsSectionController : ListSectionController , CompositionLayoutCapable{
    ...
    override func didUpdate(to object: Any) {
        ...
        if let listAdapter = self.collectionContext as? ListAdapter,
           let collectionView = listAdapter.collectionView{
            let category = IVSReelsUtility.selectedReelsCategory
            if let offset = IVSReelsUtility.scrollOffsets[category] {
    ...
    func collectionViewSectionLayout(layoutEnvironment: NSCollectionLayoutEnvironment) -> NSCollectionLayoutSection? {
        //Item
        let item = IVSCompositionalLayoutManager.createItem(width: .fractionalWidth(1.0), height: .fractionalHeight(1.0), spacing: 0)
        
        //Group
        let group = IVSCompositionalLayoutManager.createGroup(layoutDirection: .vertical, width: .fractionalWidth(1.0), height: .fractionalHeight(1.0), items: [item])
...
extension ISMReelsSectionController : ListScrollDelegate{
    func listAdapter(_ listAdapter: ListAdapter, didScroll sectionController: ListSectionController) {
        ...
        // Save the current scroll offset for the active category
              let category = IVSReelsUtility.selectedReelsCategory
        IVSReelsUtility.scrollOffsets[category] = collectionView.contentOffset
